...
class ReplicationMetadataStore @Inject constructor(val client: Client, val clusterService: ClusterService,
    ...
    suspend fun addMetadata(addReq: AddReplicationMetadataRequest): IndexResponse {
        log.info("Adding metadata for resource:${addReq.replicationMetadata.followerContext.resource},connection:${addReq.replicationMetadata.connectionName}," +
                "metadata:${addReq.replicationMetadata.metadataType}")
    ...
    suspend fun getMetadata(getMetadataReq: GetReplicationMetadataRequest): ReplicationMetadata {
        log.debug("Fetching metadata for resource:${getMetadataReq.resourceName},connection:${getMetadataReq.connectionName} " +
                "and type:${getMetadataReq.metadataType}")
        ...
        log.debug("Successfully fetched metadata for resource:${getMetadataReq.resourceName},connection:${getMetadataReq.connectionName}")
    ...
    suspend fun deleteMetadata(delMetadataReq: DeleteReplicationMetadataRequest): DeleteResponse {
        log.info("Deleting metadata for resource:${delMetadataReq.resourceName},connection:${delMetadataReq.connectionName}," +
                "metadata:${delMetadataReq.metadataType}")
    ...
    suspend fun updateMetadata(updateMetadataReq: UpdateReplicationMetadataRequest): UpdateResponse {
        log.info("Deleting metadata for resource:${updateMetadataReq.replicationMetadata.followerContext.resource}," +
                "connection:${updateMetadataReq.replicationMetadata.connectionName}," +
                "metadata:${updateMetadataReq.replicationMetadata.metadataType}")
    ...
    private suspend fun createIndex(): CreateIndexResponse {
        log.info("Creating replication store index")
