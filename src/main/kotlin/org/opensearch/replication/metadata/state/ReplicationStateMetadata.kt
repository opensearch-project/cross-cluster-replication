...
data class ReplicationStateMetadata(
    ...
    companion object {
        ...
        val replicationDetailsSerializer =
            object : NonDiffableValueSerializer<String, ReplicationStateParams>() {
                override fun write(
                    value: ReplicationStateParams,
                    out: StreamOutput,
                ) {
    ...
    class Builder {
        ...
        fun build(): ReplicationStateMetadata = ReplicationStateMetadata(replicationDetails)
    }
    ...
    fun addReplicationStateParams(
        followIndexName: String,
        replicationParams: ReplicationStateParams,
    ...
    fun removeReplicationStateParams(followIndexName: String): ReplicationStateMetadata {
    ...
    class Diff : NamedDiff<Metadata.Custom> {
        ...
        constructor(previous: ReplicationStateMetadata, current: ReplicationStateMetadata) {
            replicationDetails =
                DiffableUtils.diff(
                    previous.replicationDetails,
                    current.replicationDetails,
                    getStringKeySerializer(),
                    replicationDetailsSerializer,
                )
        }
...
fun getReplicationStateParamsForIndex(
    clusterService: ClusterService,
    followerIndex: String,
): ReplicationStateParams? =
    clusterService
        .state()
        .metadata
        .custom<ReplicationStateMetadata>(ReplicationStateMetadata.NAME)
        ?.replicationDetails
        ?.get(followerIndex)
