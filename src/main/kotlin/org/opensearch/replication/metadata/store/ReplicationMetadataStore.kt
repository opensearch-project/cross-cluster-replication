...
class ReplicationMetadataStore constructor(val client: Client, val clusterService: ClusterService,
    ...
    suspend fun getMetadata(metadataType: String, indices: List<String>): Map<String, ReplicationMetadata> {
        ...
        for (response in multiGetRes.responses) {
            ...
            result[response.response.id] = ReplicationMetadata.fromXContent(parser)
        }
        return result
    }
    ...
    suspend fun getMetadataWithVersion(metadataType: String, indices: List<String>): Map<String, GetReplicationMetadataResponse> {
        ...
        for (response in multiGetRes.responses) {
            ...
            result[response.response.id] = GetReplicationMetadataResponse(
