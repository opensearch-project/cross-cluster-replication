...
class ReplicationMetadata : ToXContent {
    ...
    companion object {
        ...
        init {
            ...
            METADATA_PARSER.declareObject(
                BiConsumer { metadata: ReplicationMetadata, context: ReplicationContext ->
                    metadata.followerContext =
                        context
                },
                ReplicationContext.REPLICATION_CONTEXT_PARSER,
                ParseField("follower_context"),
            )
            METADATA_PARSER.declareObject(
                BiConsumer { metadata: ReplicationMetadata, context: ReplicationContext ->
                    metadata.leaderContext =
                        context
                },
                ReplicationContext.REPLICATION_CONTEXT_PARSER,
                ParseField("leader_context"),
            )
            METADATA_PARSER.declareObject(
                { metadata: ReplicationMetadata, settings: Settings -> metadata.settings = settings },
        ...
        fun fromXContent(parser: XContentParser): ReplicationMetadata = METADATA_PARSER.parse(parser, null)
    }
    ...
    override fun toString(): String =
        "ReplicationMetadata - [connection_name: $connectionName, metadata_type: $metadataType, " +
            "overall_state: $overallState, reason: $reason, follower_context: ${followerContext.resource}, " +
            "leader_context: ${leaderContext.resource}, settings: $settings ]"
}
