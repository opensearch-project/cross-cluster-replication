 ...
 */
class TransportUpdateMetadataAction
    @Inject
    constructor(
    ...
    ) : TransportClusterManagerNodeAction<UpdateMetadataRequest, AcknowledgedResponse>(
            UpdateMetadataAction.NAME,
            transportService,
            clusterService,
            threadPool,
            actionFilters,
        ...
        override fun resolveIndices(request: UpdateMetadataRequest): OptionallyResolvedIndices = ResolvedIndices.of(request.indexName)
        ...
        private fun performOpenIndex(
            concreteIndices: Array<Index>,
            request: UpdateMetadataRequest,
            listener: ActionListener<AcknowledgedResponse>,
        ) {
            ...
            indexStateService.openIndex(
                updateRequest,
                object : ActionListener<OpenIndexClusterStateUpdateResponse> {
        ...
        private fun performCloseIndex(
            ...
            val closeRequest =
                CloseIndexClusterStateUpdateRequest(task.id)
                    .ackTimeout(openIndexRequest.timeout())
                    .clusterManagerNodeTimeout(openIndexRequest.clusterManagerNodeTimeout())
                    .waitForActiveShards(openIndexRequest.waitForActiveShards())
                    .indices(concreteIndices)

            indexStateService.closeIndices(
                closeRequest,
                object : ActionListener<CloseIndexResponse> {
        ...
        private fun performAliasUpdate(
            concreteIndices: Array<Index>,
            request: UpdateMetadataRequest,
            listener: ActionListener<AcknowledgedResponse>,
            indexAliasService: MetadataIndexAliasesService,
        ...
        private fun performSettingUpdate(
            concreteIndices: Array<Index>,
            request: UpdateMetadataRequest,
            listener: ActionListener<AcknowledgedResponse>,
        ...
        private fun resolveIndices(
            state: ClusterState?,
            request: UpdateMetadataRequest,
            iner: IndexNameExpressionResolver,
        ): Array<Index> {
            try {
                return iner.concreteIndices(
                    state,
                    object : IndicesRequest {
                        override fun indices(): Array<String> = arrayOf(request.indexName)

                        override fun indicesOptions(): IndicesOptions = indicesOptions
                    },
                )
            } catch (ex: IndexNotFoundException) {
                log.error(
                    "Failed to execute UpdateMetadataRequest. Index ${request.indexName} not found. type: ${request.type}: ${ex.stackTraceToString()}",
                )
                throw ex
            }
        }

        private fun performMappingUpdate(
            concreteIndices: Array<Index>,
            request: UpdateMetadataRequest,
            listener: ActionListener<AcknowledgedResponse>,
            ...
            val updateRequest =
                PutMappingClusterStateUpdateRequest(mappingRequest.source())
                    .ackTimeout(mappingRequest.timeout())
                    .clusterManagerNodeTimeout(mappingRequest.clusterManagerNodeTimeout())
                    .indices(concreteIndices)
        ...
        override fun clusterManagerOperation(
            ...
            state: ClusterState?,
            listener: ActionListener<AcknowledgedResponse>?,
        ): Unit = throw UnsupportedOperationException("The task parameter is required")
    }
