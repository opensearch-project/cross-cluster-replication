...
class ReplicationException: OpenSearchStatusException {
    ...
    constructor(message: String, shardFailures: Array<ShardOperationFailedException>):  super(message, if(shardFailures.isNotEmpty()) shardFailures[0].status() else RestStatus.INTERNAL_SERVER_ERROR, if(shardFailures.isNotEmpty()) shardFailures[0].cause else null as Throwable)  {
