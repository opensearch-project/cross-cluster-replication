...
class TransportReplicateIndexClusterManagerNodeAction
    ...
    constructor(
    ...
    ) : TransportClusterManagerNodeAction<ReplicateIndexClusterManagerNodeRequest, AcknowledgedResponse>(
            ReplicateIndexClusterManagerNodeAction.NAME,
            transportService,
            clusterService,
            threadPool,
            actionFilters,
            ::ReplicateIndexClusterManagerNodeRequest,
        ...
        override fun read(input: StreamInput): ReplicateIndexResponse = ReplicateIndexResponse(input)

        @Throws(Exception::class)
        override fun clusterManagerOperation(
            request: ReplicateIndexClusterManagerNodeRequest,
            state: ClusterState,
            ...
            log.trace(
                "Triggering relevant tasks to start replication for " +
            ...
            launch(Dispatchers.Unconfined + threadPool.coroutineContext()) {
                try {
                    if (clusterService.clusterSettings.get(ReplicationPlugin.REPLICATION_FOLLOWER_BLOCK_START)) {
                        log.debug(
                            "Replication cannot be started as " +
                    ...
                    log.debug(
                        "Response returned of the request made to get metadata of ${replicateIndexReq.leaderIndex} index on remote cluster",
                    )
                    ...
                    indexScopedSettings.validate(
                        replicateIndexReq.settings,
                        false,
                        false,
                    )

                    val params =
                        IndexReplicationParams(replicateIndexReq.leaderAlias, remoteMetadata.index, replicateIndexReq.followerIndex)

                    replicationMetadataManager.addIndexReplicationMetadata(
                        replicateIndexReq.followerIndex,
                        replicateIndexReq.leaderAlias,
                        replicateIndexReq.leaderIndex,
                        ReplicationOverallState.RUNNING,
                        user,
                        replicateIndexReq.useRoles?.getOrDefault(ReplicateIndexRequest.FOLLOWER_CLUSTER_ROLE, null),
                        replicateIndexReq.useRoles?.getOrDefault(ReplicateIndexRequest.LEADER_CLUSTER_ROLE, null),
                        replicateIndexReq.settings,
                    )

                    log.debug(
                        "Starting index replication task in persistent task service with name: replication:index:${replicateIndexReq.followerIndex}",
                    )
                    val task =
                        persistentTasksService.startTask(
                            "replication:index:${replicateIndexReq.followerIndex}",
                    ...
                    log.info(
                        "Persistent task created for replication: follower=${replicateIndexReq.followerIndex}, leader=${replicateIndexReq.leaderAlias}:${replicateIndexReq.leaderIndex}, taskId=${task.id}",
                    )
        ...
        private suspend fun getRemoteIndexMetadata(
            leaderAlias: String,
            leaderIndex: String,
        ): IndexMetadata {
        ...
        override fun checkBlock(
            request: ReplicateIndexClusterManagerNodeRequest,
            state: ClusterState,
        ): ClusterBlockException? = state.blocks.globalBlockedException(ClusterBlockLevel.METADATA_WRITE)
    }
