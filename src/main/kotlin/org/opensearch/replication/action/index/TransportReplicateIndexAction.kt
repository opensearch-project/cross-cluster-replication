...
class TransportReplicateIndexAction
    @Inject
    constructor(
                ...
                listener.completeWith {
                    ...
                    if (!request.isAutoFollowRequest) {
                        ...
                        if (!setupChecksRes.isAcknowledged) {
                            ...
                            throw org.opensearch.replication.ReplicationException(
                                "Setup checks failed while setting-up replication for ${request.followerIndex}",
                            )
                        }
                    }
                    ...
                    if (!leaderSettings.getAsBoolean(IndexSettings.INDEX_SOFT_DELETES_SETTING.key, false)) {
                        throw IllegalArgumentException(
                            "Cannot Replicate an index where the setting ${IndexSettings.INDEX_SOFT_DELETES_SETTING.key} is disabled",
                        )
                    }
...
//                ValidationUtil.checkKNNEligibility(getNodesInfoForPlugin(request.leaderAlias), request.leaderIndex)
        ...
        private suspend fun getLeaderClusterState(
            leaderAlias: String,
            leaderIndex: String,
        ): ClusterState {
        ...
        private suspend fun getLeaderIndexSettings(
            leaderAlias: String,
            leaderIndex: String,
        ): Settings {
            val remoteClient = client.getRemoteClusterClient(leaderAlias)
            ...
            val leaderSettings =
                settingsResponse.indexToSettings.get(leaderIndex)
                    ?: throw IndexNotFoundException("$leaderAlias:$leaderIndex")
            val leaderDefaultSettings =
                settingsResponse.indexToDefaultSettings.get(leaderIndex)
                    ?: throw IndexNotFoundException("$leaderAlias:$leaderIndex")
