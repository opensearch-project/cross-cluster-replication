...
class TransportUpdateAutoFollowPatternAction
    @Inject
    constructor(
    ...
    ) : HandledTransportAction<UpdateAutoFollowPatternRequest, AcknowledgedResponse>(
        ...
        override fun doExecute(
            task: Task,
            request: UpdateAutoFollowPatternRequest,
            listener: ActionListener<AcknowledgedResponse>,
        ) {
            log.info(
                "Setting-up autofollow for ${request.connection}:${request.patternName} -> " +
                    "${request.pattern}",
            )
                    ...
                    if (request.action == UpdateAutoFollowPatternRequest.Action.ADD) {
                        ...
                        if (!setupChecksRes.isAcknowledged) {
                            throw org.opensearch.replication.ReplicationException(
                                "Setup checks failed while setting-up auto follow pattern",
                            )
                        }
                    }
