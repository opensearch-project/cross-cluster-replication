...
class TransportAutoFollowClusterManagerNodeAction
    @Inject
    constructor(
        ...
        private val metadataManager: ReplicationMetadataManager,
        val indexScopedSettings: IndexScopedSettings,
            ...
            AutoFollowClusterManagerNodeAction.NAME,
            true,
            transportService,
            clusterService,
            threadPool,
            actionFilters,
            ::AutoFollowClusterManagerNodeRequest,
        ...
        override fun checkBlock(
            request: AutoFollowClusterManagerNodeRequest,
            state: ClusterState,
        ...
        override fun clusterManagerOperation(
            clusterManagerNodeReq: AutoFollowClusterManagerNodeRequest,
            state: ClusterState,
            ...
            launch(threadPool.coroutineContext()) {
                listener.completeWith {
                    ...
                    if (request.action == UpdateAutoFollowPatternRequest.Action.ADD) {
                        ...
                        metadataManager.addAutofollowMetadata(
                            request.patternName,
                            request.connection,
                            request.pattern!!,
        ...
        private suspend fun startAutoFollowTask(
            clusterAlias: String,
            patternName: String,
        ) {
            try {
                val response =
                    persistentTasksService.startTask(
                        "autofollow:$clusterAlias:$patternName",
                        AutoFollowExecutor.TASK_NAME,
                        AutoFollowParams(clusterAlias, patternName),
                    )
                if (!response.isAssigned) {
                    log.warn(
                        """Failed to assign auto follow task for cluster $clusterAlias:$patternName to any node. Check if any
                    |cluster blocks are active.
                        """.trimMargin(),
                    )
                }
        ...
        private suspend fun stopAutoFollowTask(
            clusterAlias: String,
            patternName: String,
        ) {
            try {
