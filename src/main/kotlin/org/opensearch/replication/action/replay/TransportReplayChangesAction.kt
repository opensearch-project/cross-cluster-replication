 ...
 */
class TransportReplayChangesAction
    @Inject
    constructor(
        settings: Settings,
        ...
        private val mappingUpdatedAction: MappingUpdatedAction,
            ...
            shardStateAction,
            actionFilters,
            Writeable.Reader { inp -> ReplayChangesRequest(inp) },
            Writeable.Reader { inp -> ReplayChangesRequest(inp) },
        ...
        companion object {
            ...
            private val EXECUTOR_NAME_FUNCTION =
                Function { shard: IndexShard ->
         ...
         */
        suspend fun performOnSecondary(
            request: ReplayChangesRequest,
            replicaShard: IndexShard,
        ...
        private suspend fun syncRemoteMapping(
            ...
            val getMappingsResponse =
                remoteClient.suspending(remoteClient.admin().indices()::getMappings, injectSecurityContext = true)(getMappingsRequest)
            val mappingSource =
                getMappingsResponse
                    ?.mappings()
                    ?.get(leaderIndex)
            ...
            return when (opType()!!) {
                Translog.Operation.Type.CREATE, Translog.Operation.Type.INDEX -> {
                    ...
                    Translog.Index(
                        ...
                        sourceOp.routing(),
                        sourceOp.autoGeneratedIdTimestamp,
                    )
                }
        ...
        override fun globalBlockLevel(): ClusterBlockLevel? = ClusterBlockLevel.WRITE
