...
class TransportSetupChecksAction
    @Inject
    constructor(
    ...
    ) : HandledTransportAction<SetupChecksRequest, AcknowledgedResponse>(
            SetupChecksAction.NAME,
            transportService,
            actionFilters,
            ::SetupChecksRequest,
        ) {
        companion object {
            ...
            fun unwrapSecurityExceptionIfPresent(e: Exception): Exception {
                val ex =
                    ExceptionsHelper.unwrapCausesAndSuppressed<Exception>(e) { cause ->
        ...
        override fun doExecute(
            ...
            if (request.followerContext.user != null &&
                request.followerContext.user!!
                    .roles
                    .isEmpty()
            ) {
            ...
            if (request.leaderContext.user != null &&
                request.leaderContext.user!!
                    .roles
                    .isEmpty()
            ) {
            ...
            rolePermissionsValidationAtRemote.whenComplete(
                { r ->
                    log.info(
                        "Permissions validation successful for role [connection:${request.connectionName}, " +
                ...
                { e ->
                    ...
                    log.error(
                        "Permissions validation failed for role [connection:${request.connectionName}, " +
            ...
            rolePermissionsValidationAtLocal.whenComplete(
                {
                    log.info(
                        "Permissions validation successful for User [connection:${request.connectionName}, " +
                            "resource:${request.leaderContext.resource}]",
                    )
                    triggerPermissionsValidation(
                        leaderClusterClient!!,
                        request.connectionName,
                        request.leaderContext,
                        true,
                        rolePermissionsValidationAtRemote,
                    )
                },
                { e ->
                    log.error(
                        "Permissions validation failed for role [local:$followerClusterName, " +
            ...
            userPermissionsValidationAtRemote.whenComplete(
                {
                    ...
                    triggerPermissionsValidation(
                        client,
                        followerClusterName,
                        request.followerContext,
                        true,
                        rolePermissionsValidationAtLocal,
                    )
                },
                { e ->
                    var exceptionToThrow = e
                    ...
                    log.error(
                        "Permissions validation failed for User [connection:${request.connectionName}, " +
                            "resource:${request.leaderContext.resource}] with ${exceptionToThrow.stackTraceToString()}",
                    )
                    listener.onFailure(unwrapSecurityExceptionIfPresent(exceptionToThrow))
                },
            ...
            userPermissionsValidationAtLocal.whenComplete(
                {
                    log.info(
                        "Permissions validation successful for User [local:$followerClusterName, " +
                    ...
                    triggerPermissionsValidation(
                        ...
                        request.leaderContext,
                        false,
                        userPermissionsValidationAtRemote,
                    )
                },
                { e ->
                    log.error(
                        "Permissions validation failed for User [local:$followerClusterName, " +
                            "resource:${request.followerContext.resource}] with ${e.stackTraceToString()}",
                    )
                    listener.onFailure(unwrapSecurityExceptionIfPresent(e))
                },
        ...
        private fun triggerPermissionsValidation(
            client: Client,
            cluster: String,
