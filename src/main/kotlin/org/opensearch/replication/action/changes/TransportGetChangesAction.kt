...
class TransportGetChangesAction
    @Inject
    constructor(
        ...
        override fun shardOperation(
            ...
            shardId: ShardId,
        ): GetChangesResponse = throw UnsupportedOperationException("use asyncShardOperation")

        @Suppress("BlockingMethodInNonBlockingContext")
        override fun asyncShardOperation(
            request: GetChangesRequest,
            shardId: ShardId,
            listener: ActionListener<GetChangesResponse>,
        ) {
            ...
            GlobalScope.launch(threadPool.coroutineContext(REPLICATION_EXECUTOR_NAME_LEADER)) {
                ...
                listener.completeWith {
                    ...
                    if (lastGlobalCheckpoint(indexShard, isRemoteEnabledOrMigrating) < request.fromSeqNo) {
                        ...
                        if (lastGlobalCheckpoint(indexShard, isRemoteEnabledOrMigrating) < request.fromSeqNo) {
                            assert(gcp > lastGlobalCheckpoint(indexShard, isRemoteEnabledOrMigrating)) {
                                "Checkpoint didn't advance at all $gcp ${lastGlobalCheckpoint(indexShard, isRemoteEnabledOrMigrating)}"
                            }
                    ...
                    GetChangesResponse(
                        ops,
                        request.fromSeqNo,
                        indexShard.maxSeqNoOfUpdatesOrDeletes,
                        lastGlobalCheckpoint(indexShard, isRemoteEnabledOrMigrating),
                    )
                }
            }
        }

        private fun lastGlobalCheckpoint(
            indexShard: IndexShard,
            isRemoteEnabledOrMigrating: Boolean,
        ): Long {
        ...
        private fun isTranslogPruningByRetentionLeaseEnabled(shardId: ShardId): Boolean {
            val enabled =
                clusterService
                    .state()
                    .metadata.indices
                    .get(shardId.indexName)
                    ?.settings
                    ?.getAsBoolean(REPLICATION_INDEX_TRANSLOG_PRUNING_ENABLED_SETTING.key, false)
            if (enabled != null) {
                return enabled
            }
            return false
        ...
        override fun resolveIndex(request: GetChangesRequest): Boolean = true

        override fun getResponseReader(): Writeable.Reader<GetChangesResponse> =
            Writeable.Reader { inp: StreamInput -> GetChangesResponse(inp) }
