...
class TransportResumeIndexReplicationAction
    @Inject
    constructor(
        ...
        override fun clusterManagerOperation(
            ...
            launch(Dispatchers.Unconfined + threadPool.coroutineContext()) {
                listener.completeWith {
                    ...
                    val leaderSettings =
                        settingsResponse.indexToSettings.get(params.leaderIndex.name)
                            ?: throw IndexNotFoundException(params.leaderIndex.name)
        ...
        private suspend fun isResumable(params: IndexReplicationParams): Boolean {
            ...
            shards?.forEach {
                ...
                if (!retentionLeaseHelper.verifyRetentionLeaseExist(ShardId(params.leaderIndex, followerShardId.id), followerShardId)) {
                    log.info(
                        "Retention lease missing for follower=${params.followerIndexName}, followerShard=$followerShardId, leaderShard=${ShardId(
                            params.leaderIndex,
                            followerShardId.id,
                        )} - replication cannot be resumed",
                    )
                    isResumable = false
                }
            }
