...
class TransportGetFileChunkAction
    @Inject
    constructor(
        ...
        override fun shardOperation(
            ...
            store.performOp({
                ...
                val currentInput =
                    restoreLeaderService.openInputStream(
                        request.restoreUUID,
                        request,
                        fileMetaData.name(),
                        fileMetaData.length(),
        ...
        override fun resolveIndex(request: GetFileChunkRequest): Boolean = true

        override fun getResponseReader(): Writeable.Reader<GetFileChunkResponse> =
            Writeable.Reader { inp: StreamInput -> GetFileChunkResponse(inp) }
