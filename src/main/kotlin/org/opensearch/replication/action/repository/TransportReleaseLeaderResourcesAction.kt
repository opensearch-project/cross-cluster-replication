...
class TransportReleaseLeaderResourcesAction
    @Inject
    constructor(
    ...
    ) : TransportSingleShardAction<ReleaseLeaderResourcesRequest, AcknowledgedResponse>(
            ReleaseLeaderResourcesAction.NAME,
            threadPool,
            clusterService,
            transportService,
            actionFilters,
        ...
        override fun shardOperation(
            request: ReleaseLeaderResourcesRequest,
            shardId: ShardId,
        ...
        override fun resolveIndex(request: ReleaseLeaderResourcesRequest?): Boolean = true

        override fun getResponseReader(): Writeable.Reader<AcknowledgedResponse> =
            Writeable.Reader { inp: StreamInput -> AcknowledgedResponse(inp) }

        override fun shards(
            state: ClusterState,
            request: InternalRequest,
        ): ShardsIterator? = state.routingTable().shardRoutingTable(request.request().leaderShardId).primaryShardIt()
    }
