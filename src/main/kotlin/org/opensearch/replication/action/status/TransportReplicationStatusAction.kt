...
class TransportReplicationStatusAction
    ...
    constructor(
        ...
        override fun doExecute(
            ...
            listener: ActionListener<ReplicationStatusResponse>,
        ) {
                    ...
                    try {
                        ...
                        var followerResponse =
                            client.suspendExecute(
                                ...
                                ShardInfoRequest(metadata.followerContext.resource),
                                true,
                            )
                        ...
                        var leaderResponse =
                            remoteClient.suspendExecute(
                                ShardsInfoAction.INSTANCE,
                                ShardInfoRequest(metadata.leaderContext.resource),
                                true,
                            )
        ...
        private fun populateAggregatedResponse(followerResponse: ReplicationStatusResponse) {
            ...
            followerResponse.shardInfoResponse.forEach {
                ...
                if (it.isRestoreDetailsInitialized()) {
                    ...
                    aggregateRecovereyPercentage =
                        (numberOfShardsiInRestore * aggregateRecovereyPercentage + it.restoreDetails.recovereyPercentage) /
                        (numberOfShardsiInRestore + 1)
                    aggregateFileRecovereyPercentage =
                        (numberOfShardsiInRestore * aggregateFileRecovereyPercentage + it.restoreDetails.fileRecovereyPercentage) /
                        (numberOfShardsiInRestore + 1)
            ...
            if (anyShardInReplay) {
                followerResponse.aggregatedReplayDetails =
                    ReplayDetails(aggregatedRemoteCheckpoint, aggregatedLocalCheckpoint, aggregatedSeqNo)
            }
            if (anyShardInRestore) {
                followerResponse.aggregatedRestoreDetails =
                    RestoreDetails(
                        aggregateTotalBytes,
                        aggregateRecoveredBytes,
                        aggregateRecovereyPercentage,
                        aggregateTotalFiles,
                        aggregateRecoveredFiles,
                        aggregateFileRecovereyPercentage,
                        startTime,
                        time,
                    )
