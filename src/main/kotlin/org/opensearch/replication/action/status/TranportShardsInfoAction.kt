...
class TranportShardsInfoAction @Inject constructor(
    ...
    override fun shardOperation(request: ShardInfoRequest, shardRouting: ShardRouting): ShardInfoResponse {
        ...
        if (indexShard.recoveryState().recoverySource.type.equals(RecoverySource.Type.SNAPSHOT) and
            ...
            return ShardInfoResponse(
                shardRouting.shardId(), ShardInfoResponse.BOOTSTRAPPING,
                RestoreDetails(
                    indexState.totalBytes(), indexState.recoveredBytes(),
        ...
        return ShardInfoResponse(
            shardRouting.shardId(), ShardInfoResponse.SYNCING,
            ReplayDetails(
    ...
    override fun checkRequestBlock(state: ClusterState, request: ShardInfoRequest?, concreteIndices: Array<String?>?): ClusterBlockException? {
        return null
    }
