...
class TranportShardsInfoAction
    @Inject
    constructor(
        ...
        private val indicesService: IndicesService,
            ...
            ShardInfoResponse,
        >(
            ShardsInfoAction.NAME,
            clusterService,
            transportService,
            actionFilters,
            indexNameExpressionResolver,
            Writeable.Reader { X -> ShardInfoRequest(X) },
        ...
        override fun readShardResult(si: StreamInput): ShardInfoResponse? = ShardInfoResponse(si)
        ...
        override fun newResponse(
            ...
            (ReplicationStatusResponse(totalShards, successfulShards, failedShards, shardFailures, shardInfoRespons))

        @Throws(IOException::class)
        override fun readRequestFrom(si: StreamInput): ShardInfoRequest = ShardInfoRequest(si)
        ...
        override fun shardOperation(
            ...
            if (indexShard
                    .recoveryState()
                    .recoverySource.type
                    .equals(RecoverySource.Type.SNAPSHOT) and
                (indexState.recoveredBytesPercent() < 100)
            ) {
                return ShardInfoResponse(
                    shardRouting.shardId(),
                    ShardInfoResponse.BOOTSTRAPPING,
                    RestoreDetails(
                        indexState.totalBytes(),
                        indexState.recoveredBytes(),
                        indexState.recoveredBytesPercent(),
                        indexState.totalFileCount(),
                        indexState.recoveredFileCount(),
                        indexState.recoveredFilesPercent(),
                        indexState.startTime(),
                        indexState.time(),
                    ),
                )
            }
        ...
        override fun checkRequestBlock(
            state: ClusterState,
            request: ShardInfoRequest?,
            concreteIndices: Array<String?>?,
        ): ClusterBlockException? = null

        override fun checkGlobalBlock(
            state: ClusterState?,
            request: ShardInfoRequest?,
        ): ClusterBlockException? = null
    }
