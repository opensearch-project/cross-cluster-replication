...
class ShardReplicationTask(
        ...
        try {
            ...
            if (null != downstreamException) {
                ...
                withContext(NonCancellable) {
                    logInfo(
                        "Going to mark ShardReplicationTask as Failed: leaderShard=$leaderShardId, followerShard=$followerShardId, cause=${throwable.stackTraceToString()}",
                    )
                    try {
    ...
    private suspend fun replicate(scope: CoroutineScope) {
        ...
        logInfo(
            "Shard replication started: leaderShard=$leaderShardId, followerShard=$followerShardId, localCheckpoint=${indexShard.localCheckpoint}, globalCheckpoint=${indexShard.lastSyncedGlobalCheckpoint}",
        )
        ...
        try {
            ...
            log.debug(
                "${Thread.currentThread().name}: Initial retention lease renewed: leaderShard=$leaderShardId, followerShard=$followerShardId, seqNo=${indexShard.lastSyncedGlobalCheckpoint + 1}",
            )
        } catch (ex: Exception) {
            ...
            log.error(
                "Retention lease renewal failed: leaderShard=$leaderShardId, followerShard=$followerShardId, error=${ex.stackTraceToString()}",
            )
        }
        ...
        val sequencer =
            TranslogSequencer(
                ...
                TaskId(clusterService.nodeName, id),
                client,
            ...
            while (isActive) {
                ...
                launch {
                    ...
                    try {
                        ...
                        logInfo(
                            "Node not connected to $leaderAlias, retrying with different node. followerShard=$followerShardId, seqNo=$fromSeqNo: ${e.message}",
                        )
                        ...
                        if (e is IllegalArgumentException &&
                            ...
                            logError(
                                "Hit 2GB limit with current batch size ${changeTracker.batchSizeSettings().getEffectiveBatchSize()}. Reducing batch size.",
                            )
                            changeTracker.reduceBatchSize()
                            logError(
                                "Reduced batch size to ${changeTracker.batchSizeSettings().getEffectiveBatchSize()}. Retrying immediately.",
                            )
                            changeTracker.updateBatchFetched(false, fromSeqNo, toSeqNo, fromSeqNo - 1, -1)
                ...
                try {
                    ...
                    log.debug(
                        "${Thread.currentThread().name}: Retention lease renewed: leaderShard=$leaderShardId, followerShard=$followerShardId, seqNo=${indexShard.lastSyncedGlobalCheckpoint + 1}",
                    )
                } catch (ex: Exception) {
                    when (ex) {
                        ...
                        is RetentionLeaseInvalidRetainingSeqNoException -> {
                            if (System.currentTimeMillis() - lastLeaseRenewalMillis >
                                ...
                                log.error(
                                    "Retention lease renewal has been failing for last " +
                            ...
                            logInfo("Exception renewing retention lease. Not an issue - ${ex.stackTraceToString()}")
                        }
                    }
                }
            }
    ...
    private suspend fun getChanges(
        ...
        var changesResp =
            remoteClient.suspendExecuteWithRetries(
    ...
    override fun toString(): String = "ShardReplicationTask(from=${leaderAlias}$leaderShardId to=$followerShardId)"
