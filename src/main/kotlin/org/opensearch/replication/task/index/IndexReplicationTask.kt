...
open class IndexReplicationTask(
    id: Long,
    type: String,
    action: String,
    description: String,
    ...
    private val retentionLeaseHelper =
        RemoteClusterRetentionLeaseHelper(clusterService.clusterName.value(), clusterService.state().metadata.clusterUUID(), remoteClient)
                    ...
                    when (currentTaskState.state) {
                        ReplicationState.INIT -> {
                            ...
                            if (isResumed()) {
                                ...
                                log.info(
                                    "Starting new index replication: follower=$followerIndexName, leader=$leaderAlias:${leaderIndex.name}",
                                )
                                    ...
                                    is FailedState -> {
                                        ...
                                        if (replicationSettings.replicateIndexDeletion &&
                                            state.errorMsg.contains(
                                                "org.opensearch.index.IndexNotFoundException - \"no such index [" +
                                                    leaderIndex.name + "]\"",
                                            )
                                        ) {
    ...
    private suspend fun failReplication(failedState: FailedState) {
        withContext(NonCancellable) {
            ...
            log.error(
                "Moving replication[IndexReplicationTask:$id][follower=$followerIndexName, leader=$leaderAlias:${leaderIndex.name}, reason=$reason] to failed state",
            )
            try {
    ...
    private fun isResumed(): Boolean = clusterService.state().routingTable.hasIndex(followerIndexName)
     ...
     */
    private suspend fun conditionallyOpenIndex(
        indexName: String,
        logSuffix: String = "",
    ) {
    ...
    private suspend fun evalMonitoringState(): IndexReplicationState {
        ...
        val followerShardIds =
            clusterService
                .state()
                .routingTable
                .indicesRouting()
                .get(followerIndexName)
                ?.shards()
                ?.map { shard -> shard.value.shardId }
                ?.stream()
                ?.collect(Collectors.toSet())
                .orEmpty()
        val runningShardTasksForIndex =
            persistentTasks
        ...
        if (runningShardTasksForIndex.size != followerShardIds.size) {
            ...
            log.info(
                "Shard task count mismatch for follower=$followerIndexName: expected=${followerShardIds.size}, running=${runningShardTasksForIndex.size}, missingShards=$missingShardsIds",
            )
    ...
    private suspend fun updateFollowerMapping(
        ...
        while (scope.isActive) {
            try {
                ...
                var settingsResponse =
                    remoteClient.suspending(remoteClient.admin().indices()::getSettings, injectSecurityContext = true)(gsr)
                ...
                leaderSettings =
                    leaderSettings.filter { k: String ->
                ...
                followerSettings =
                    followerSettings.filter { k: String ->
                ...
                var getAliasesRes =
                    remoteClient.suspending(remoteClient.admin().indices()::getAliases, injectSecurityContext = true)(getAliasesRequest)
                ...
                var mappingResponse =
                    remoteClient.suspending(remoteClient.admin().indices()::getMappings, injectSecurityContext = true)(gmr)
                var leaderMappingSource =
                    mappingResponse
                        ?.mappings
                        ?.get(this.leaderIndex.name)
                        ?.source()
                        ?.toString()

                @Suppress("UNCHECKED_CAST")
                val leaderProperties =
                    mappingResponse
                        ?.mappings()
                        ?.get(
                            this.leaderIndex.name,
                        )?.sourceAsMap()
                        ?.toMap()
                ...
                @Suppress("UNCHECKED_CAST")
                val followerProperties =
                    mappingResponse
                        ?.mappings()
                        ?.get(
                            this.followerIndexName,
                        )?.sourceAsMap()
                        ?.toMap()
                        ?.get("properties") as? Map<String, Any>?
                ...
                log.error(
                    "Error in getting the required metadata follower=$followerIndexName, leader=$leaderAlias:${leaderIndex.name}, error=${e.stackTraceToString()}",
                )
            } finally {
                withContext(NonCancellable) {
            ...
            try {
                ...
                var updateRequest =
                    UpdateMetadataRequest(
                        ...
                        Requests.closeIndexRequest(followerIndexName),
                    )
                client.suspendExecute(UpdateMetadataAction.INSTANCE, updateRequest, injectSecurityContext = true)
    ...
    private suspend fun pauseReplication(state: FailedState): IndexReplicationState {
        try {
            log.error(
                "Going to initiate auto-pause of $followerIndexName due to shard failures - failedShards=${state.failedShards.keys}, reason=${state.errorMsg}",
            )
            val pauseReplicationResponse =
                client.suspendExecute(
                    replicationMetadata,
                    PauseIndexReplicationAction.INSTANCE,
                    PauseIndexReplicationRequest(followerIndexName, "$AUTOPAUSED_REASON_PREFIX + ${state.errorMsg}"),
                    defaultContext = true,
                )
    ...
    private suspend fun deleteIndex() {
        try {
            log.info(
                "Going to initiate deletion of index $followerIndexName due to deletion of corresponding leader index ${leaderIndex.name}",
            )
            val deleteIndexResponse =
                client.suspendExecute(
                    replicationMetadata,
                    DeleteIndexAction.INSTANCE,
                    DeleteIndexRequest(followerIndexName),
                    defaultContext = true,
                )
    ...
    private fun findAllReplicationFailedShardTasks(
    ...
    private suspend fun updateState(newState: IndexReplicationState): IndexReplicationState =
        suspendCoroutine { cont ->
            updatePersistentTaskState(
                newState,
                object : ActionListener<PersistentTask<*>> {
        ...
        val tasks =
            shards
                ?.map {
                    ...
                    val task =
                        runningShardTasks.getOrElse(shardId) {
                            log.info(
                                "Starting missing shard task for follower=$followerIndexName, shardId=$shardId, leaderShard=${ShardId(
                                    leaderIndex,
                                    shardId.id,
                                )}",
                            )
    ...
    private suspend fun setupAndStartRestore(): IndexReplicationState {
        ...
        val updateSettingsRequest =
            remoteClient
                ...
                .request()
        val updateResponse =
            remoteClient.suspending(remoteClient.admin().indices()::updateSettings, injectSecurityContext = true)(updateSettingsRequest)
        ...
        if (leaderIndex.name != followerIndexName) {
            restoreRequest
                .renamePattern(leaderIndex.name)
    ...
    private suspend fun waitForRestore(): IndexReplicationState {
        ...
        while (restore != null && restore.state() != RestoreInProgress.State.FAILURE &&
            restore.state() != RestoreInProgress.State.SUCCESS
        ) {
            try {
                ...
                log.debug(
                    "Timed out while waiting for restore to complete. follower=$followerIndexName, leader=$leaderAlias:${leaderIndex.name}, elapsedSecs=$elapsedSecs, timeoutCount=$timeoutCount, restoreState=${restore?.state()}",
                )
            }
        ...
        if (restore == null) {
            ...
            if (doesValidIndexExists()) {
                ...
                return FailedState(
                    ...
                    This can happen if there was a badly timed cluster manager node failure.
                    """.trimIndent(),
                )
            }
    ...
    private fun doesValidIndexExists(): Boolean {
        try {
            client
                .admin()
                .indices()
                .prepareGetSettings(followerIndexName)
                .get()
            ...
            val recoveries =
                client
                    .admin()
                    .indices()
                    .prepareRecoveries(followerIndexName)
                    .get()
                    .shardRecoveryStates()
                    .get(followerIndexName)
            val activeRecoveries =
                recoveries
                    ?.stream()
                    ?.filter(RecoveryState::getPrimary)
                    ?.filter { r ->
                        r.stage != RecoveryState.Stage.DONE
                    }?.toList()
    ...
    private fun inProgressRestore(cs: ClusterState): RestoreInProgress.Entry? =
        cs.custom<RestoreInProgress>(RestoreInProgress.TYPE).singleOrNull { entry ->
    ...
    private suspend fun startReplicationTask(replicationParams: ShardReplicationParams): PersistentTask<ShardReplicationParams> =
        persistentTasksService.startTask(
            ShardReplicationTask.taskIdForShard(replicationParams.followerShardId),
            ShardReplicationExecutor.TASK_NAME,
            replicationParams,
        )

    override fun onIndexShardClosed(
        shardId: ShardId,
    ...
    override fun replicationTaskResponse(): CrossClusterReplicationTaskResponse = IndexReplicationTaskResponse(currentTaskState)

    class IndexReplicationTaskResponse(
        private val taskState: IndexReplicationState,
    ) : CrossClusterReplicationTaskResponse(ReplicationState.COMPLETED.name),
        ToXContentObject {
