    ...
    fun run(initialState: PersistentTaskState? = null) {
        overallTaskScope.launch {
            ...
            try {
                ...
                log.error(
                    "Exception encountered in CrossClusterReplicationTask - task=${this.javaClass.simpleName}, id=$id, follower=$followerIndexName, leader=$leaderAlias, coroutine:isActive=$isActive Context=$coroutineContext",
                    e,
                )
        ...
        if (this::replicationMetadata.isInitialized) {
            ...
            while (overallTaskScope.isActive) {
                try {
                    ...
                    log.error(
                        "Failed to fetch replication metadata due to task=${this.javaClass.simpleName}, id=$id, error=${e.message}",
                        e,
                    )
