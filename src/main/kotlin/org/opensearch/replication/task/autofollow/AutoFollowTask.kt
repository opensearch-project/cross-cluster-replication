...
class AutoFollowTask(
    ...
    override suspend fun execute(
        ...
        log.info(
            "AutoFollow task started: leaderAlias=$leaderAlias, pattern=${params.patternName}, resource=${replicationMetadata.leaderContext.resource}",
        )
    ...
    private fun addRetryScheduler() {
        ...
        try {
            ...
            log.error(
                "Error scheduling retry on failed autofollow indices leaderAlias=$leaderAlias, pattern=$patternName, error=${e.stackTraceToString()}",
            )
            retryScheduler = null
        }
    }
    ...
    private suspend fun pollForIndices() {
        ...
        try {
            ...
            if (stat.failedLeaderCall >= 0 && stat.failedLeaderCall.rem(10) == 0L) {
                log.error(
                    "Fetching remote indices failed with error - leaderAlias=$leaderAlias, pattern=$patternName, consecutiveFailures=${stat.failedLeaderCall}, error=${e.stackTraceToString()}",
                )
            }
        ...
        var currentIndices =
            clusterService
                ...
                .metadata()
                .concreteAllIndices
                .asIterable() // All indices - open and closed on the cluster
        if (remoteIndices.intersect(currentIndices).isNotEmpty()) {
            ...
            if (currentIndices.toSet() != trackingIndicesOnTheCluster) {
                log.info(
                    "Cannot initiate replication for the following indices from leader ($leaderAlias) as indices with " +
    ...
    private suspend fun startReplication(leaderIndex: String) {
        if (clusterService.state().metadata().hasIndex(leaderIndex)) {
            log.info(
                """Cannot replicate $leaderAlias:$leaderIndex as an index with the same name already 
                        |exists.
                """.trimMargin(),
            )
            return
        ...
        try {
            ...
            val followerRole =
                replicationMetadata.followerContext.user
                    ?.roles
                    ?.get(0)
            val leaderRole =
                replicationMetadata.leaderContext.user
                    ?.roles
                    ?.get(0)
    ...
    override fun toString(): String = "AutoFollowTask(from=$leaderAlias with pattern=${params.patternName})"
    ...
    override fun getStatus(): AutoFollowStat = stat
