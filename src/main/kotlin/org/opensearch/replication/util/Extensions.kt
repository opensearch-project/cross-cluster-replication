...
fun <T : ActionResponse> Client.execute(
...
fun <T> Client.executeBlockUnderSecurityContext(
...
fun IndexRequestBuilder.execute(
    id: String,
    ...
    repeat(numberOfRetries - 1) { index ->
        try {
            ...
            return suspendExecute(
                ...
                action,
                req,
                injectSecurityContext = injectSecurityContext,
        ...
        log.warn(
            "Encountered a failure(can be ignored) while getting changes:  OpenSearchTimeoutException. " +
                "Retrying in ${currentBackoff / 1000} seconds.",
        )
    ...
    repeat(numberOfRetries) {
        try {
            ...
            if (retryOn.contains(e.javaClass) && retryCount < numberOfRetries) {
                ...
                log.error(
                    "Restore of shard from remote cluster repository failed permanently after all retries due to ${e.stackTraceToString()}",
                )
                store.decRef()
...
fun User.toInjectedUser(): String? = "${SecurityContext.REPLICATION_PLUGIN_USER}|${backendRoles.joinToString(separator = ",")}"

fun User.toInjectedRoles(): String? = "${SecurityContext.REPLICATION_PLUGIN_USER}|${roles.joinToString(separator = ",")}"
