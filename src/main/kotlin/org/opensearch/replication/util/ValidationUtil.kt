...
object ValidationUtil {
    ...
    fun validateAnalyzerSettings(
        ...
        for (analyserSetting in analyserSettings.keySet()) {
            val settingValue =
                if (overriddenSettings.hasValue(
                        analyserSetting,
                    )
                ) {
                    overriddenSettings.get(analyserSetting)
                } else {
                    analyserSettings.get(analyserSetting)
                }
    ...
    fun validateName(
        ...
        if (!Strings.validFileName(name)) {
            validationException.addValidationError(
                "Value $name must not contain the following characters ${Strings.INVALID_FILENAME_CHARS}",
            )
        }
        ...
        try {
            ...
            if (byteCount > MetadataCreateIndexService.MAX_INDEX_NAME_BYTES) {
                validationException.addValidationError(
                    "Value $name must not be longer than ${MetadataCreateIndexService.MAX_INDEX_NAME_BYTES} bytes",
                )
            }
    ...
    fun validatePattern(
        ...
        if (!Strings.validFileNameExcludingAstrix(pattern)) {
            validationException.addValidationError(
                "Autofollow pattern: $pattern must not contain the following characters ${Strings.INVALID_FILENAME_CHARS}",
            )
        }
    ...
    private fun validateLeaderIndexMetadata(leaderIndexMetadata: IndexMetadata) {
        if (Version.CURRENT.before(leaderIndexMetadata.creationVersion)) {
            val err =
                "Leader index[${leaderIndexMetadata.index.name}] is on " +
        ...
        if (Version.CURRENT.before(leaderIndexMetadata.upgradedVersion)) {
            val err =
                "Leader index[${leaderIndexMetadata.index.name}] is upgraded with " +
     ...
     */
    fun validateLeaderIndexState(
        leaderAlias: String,
        leaderIndex: String,
        leaderClusterState: ClusterState,
    ) {
        val leaderIndexMetadata =
            leaderClusterState.metadata.index(leaderIndex) ?: throw IndexNotFoundException("$leaderAlias:$leaderIndex")
    ...
    fun checkKNNEligibility(
        ...
        if (!(
            ...
            throw IllegalStateException(
                "Cannot proceed with replication for k-NN enabled index $leaderIndex as knn plugin is not installed.",
            )
        }
    }
    ...
    fun isRemoteStoreEnabledCluster(clusterService: ClusterService): Boolean =
        clusterService.settings
            .getByPrefix(
                Node.NODE_ATTRIBUTES.key + RemoteStoreNodeAttribute.REMOTE_STORE_NODE_ATTRIBUTE_KEY_PREFIX,
            ).isEmpty ==
            false

    fun isRemoteEnabledOrMigrating(clusterService: ClusterService): Boolean =
        isRemoteStoreEnabledCluster(clusterService) ||
