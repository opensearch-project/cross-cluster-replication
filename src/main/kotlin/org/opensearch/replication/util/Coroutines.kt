...
suspend fun <Req : ActionRequest, Resp : ActionResponse> OpenSearchClient.suspendExecute(
    ...
    withContext(this.threadPool().coroutineContext(null, action.name(), injectSecurityContext, defaultContext)) {
...
suspend fun <Req : ActionRequest, Resp : ActionResponse> OpenSearchClient.suspendExecute(
    ...
    withContext(this.threadPool().coroutineContext(replicationMetadata, action.name(), true, defaultContext)) {
...
suspend fun IndexShard.waitForGlobalCheckpoint(
    waitingForGlobalCheckpoint: Long,
    timeout: TimeValue?,
): Long =
    suspendCancellableCoroutine { cont ->
        val listener =
            object : GlobalCheckpointListeners.GlobalCheckpointListener {
...
suspend fun ClusterStateObserver.waitForNextChange(
    reason: String,
    predicate: (ClusterState) -> Boolean = { true },
) = suspendCancellableCoroutine { cont ->
    waitForNextChange(
...
): T = suspendCoroutine { cont -> submitStateUpdateTask(source, updateTaskFactory(CoroutineActionListener(cont))) }

suspend fun <T : PersistentTaskParams> PersistentTasksService.startTask(
    taskId: String,
    taskName: String,
    params: T,
): PersistentTask<T> = suspendCoroutine { cont -> this.sendStartRequest(taskId, taskName, params, CoroutineActionListener(cont)) }

suspend fun PersistentTasksService.removeTask(taskId: String): PersistentTask<*> =
    suspendCoroutine { cont -> this.sendRemoveRequest(taskId, CoroutineActionListener(cont)) }
...
suspend fun PersistentTasksService.waitForTaskCondition(
    ...
    suspendCancellableCoroutine { cont ->
        val listener =
            object : PersistentTasksService.WaitForPersistentTaskListener<PersistentTaskParams> {
...
class CoroutineActionListener<T>(
 ...
 */
class OpenSearchThreadContextElement(
    ...
    suspendCoroutine { continuation ->
        clusterService.submitStateUpdateTask(
            ...
            object : ClusterStateTaskListener {
                ...
                override fun clusterStateProcessed(
                    source: String?,
