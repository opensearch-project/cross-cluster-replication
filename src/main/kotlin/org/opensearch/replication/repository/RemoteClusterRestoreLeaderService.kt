 ...
 */
@Singleton
class RemoteClusterRestoreLeaderService
    @Inject
    constructor(
        ...
        ): RestoreContext = onGoingRestores.getOrPut(restoreUUID) { constructRestoreContext(restoreUUID, request) }

        private fun getLeaderClusterRestore(restoreUUID: String): RestoreContext =
            onGoingRestores[restoreUUID] ?: throw IllegalStateException("missing restoreContext")
        ...
        fun <T : SingleShardRequest<T>?> openInputStream(
            ...
            val leaderIndexShard =
                indicesService.getShardOrNull(request.leaderShardId)
