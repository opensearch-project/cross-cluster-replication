...
class RemoteClusterRepository(
    ...
    override fun getRestoreThrottleTimeInNanos(): Long = restoreRateLimitingTimeInNanos.count()

    override fun getRemoteUploadThrottleTimeInNanos(): Long = throw UnsupportedOperationException("Operation not permitted")

    override fun getRemoteDownloadThrottleTimeInNanos(): Long = throw UnsupportedOperationException("Operation not permitted")

    override fun getLowPriorityRemoteDownloadThrottleTimeInNanos(): Long = throw UnsupportedOperationException("Operation not permitted")
    ...
    ): Unit = throw UnsupportedOperationException("Operation not permitted")
    ...
    override fun startVerification(): String = throw UnsupportedOperationException("Operation not permitted")
    ...
    override fun getMetadata(): RepositoryMetadata = repositoryMetadata
    ...
    override fun cloneShardSnapshot(
        source: SnapshotId?,
        target: SnapshotId?,
        shardId: RepositoryShardId?,
    ...
    override fun getSnapshotThrottleTimeInNanos(): Long = throw UnsupportedOperationException("Operation not permitted")
    ...
    override fun getShardSnapshotStatus(
        ...
        for (i in indicesStatsResponse.shards.indices) {
            if (indicesStatsResponse.shards[i]
                    .shardRouting
                    .shardId()
                    .id == shardId.id
            ) {
                val sizeInBytes =
                    indicesStatsResponse.shards[i]
                        .stats
                        ?.store
                        ?.sizeInBytes!!
     ...
     */
    override fun getSnapshotIndexMetaData(
    ...
    override fun restoreShard(
        ...
        launch(Dispatchers.IO + leaderClusterClient.threadPool().coroutineContext()) {
            ...
            restoreShardWithRetries(
                ...
                recoveryState,
                listener,
                ::restoreShardUsingMultiChunkTransfer,
                log = log,
            )
    ...
    suspend fun restoreShardUsingMultiChunkTransfer(
        ...
        val leaderShardRouting =
            leaderClusterState.routingTable
                .shardRoutingTable(
        ...
        val getStoreMetadataRequest =
            GetStoreMetadataRequest(
                restoreUUID,
                leaderShardNode,
                leaderShardId,
                RemoteClusterRetentionLeaseHelper.getFollowerClusterNameWithUUID(
        ...
        log.info(
            "Starting bootstrap restore: follower=$followerIndexName, followerShard=$followerShardId, leaderShard=$leaderShardId, fileCount=${fileMetadata.size}, totalSizeBytes=$totalSizeBytes",
        )
        multiChunkTransfer =
            RemoteClusterMultiChunkTransfer(
                ...
                store,
                replicationSettings.concurrentFileChunks,
                restoreUUID,
                replMetadata,
                leaderShardNode,
                leaderShardId,
                ...
                leaderClusterClient,
                recoveryState,
                replicationSettings.chunkSize,
                object : ActionListener<Void> {
                    override fun onFailure(e: java.lang.Exception?) {
                        log.error(
                            "Restore of ${store.shardId()} failed due to follower=$followerIndexName, leaderShard=$leaderShardId, error=${e?.stackTraceToString()}",
                        )
                        if (e is NodeDisconnectedException || e is NodeNotConnectedException || e is ConnectTransportException) {
                            ...
                            log.error(
                                "Not retrying restore shard for ${store.shardId()} follower=$followerIndexName, leaderShard=$leaderShardId",
                            )
                            store.decRef()
    ...
    private fun releaseLeaderResources(
        ...
        try {
            val releaseResourcesReq =
                ReleaseLeaderResourcesRequest(
                    restoreUUID,
                    leaderShardNode,
                    leaderShardId,
                    clusterService.clusterName.value(),
    ...
    override fun isSystemRepository(): Boolean = throw UnsupportedOperationException("Operation not permitted")

    /*
     * This method makes a blocking call to the leader cluster
     * For restore workflow this is expected.
     */
    private fun getLeaderClusterState(
        includeNodes: Boolean,
        includeRoutingTable: Boolean,
        vararg remoteIndices: String,
    ): ClusterState {
    ...
    private fun getReplicationMetadata(followerIndex: String): ReplicationMetadata =
        replicationMetadataManager.getIndexReplicationMetadata(
            followerIndex,
            repositoryMetadata.leaderClusterName(),
