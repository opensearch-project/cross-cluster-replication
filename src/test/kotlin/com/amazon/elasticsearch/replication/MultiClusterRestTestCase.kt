...
abstract class MultiClusterRestTestCase : ESTestCase() {
    ...
    fun docCount(cluster: RestHighLevelClient, indexName: String) : Int {
        val persistentConnectionRequest = Request("GET", "/$indexName/_search?pretty&q=*")

        val persistentConnectionResponse = cluster.lowLevelClient.performRequest(persistentConnectionRequest)
        val statusResponse: Map<String, Map<String, Map<String, Any>>> = ESRestTestCase.entityAsMap(persistentConnectionResponse) as Map<String, Map<String, Map<String, String>>>
        return statusResponse["hits"]?.get("total")?.get("value") as Int
    }

    fun deleteIndex(testCluster: RestHighLevelClient, indexName: String) {
