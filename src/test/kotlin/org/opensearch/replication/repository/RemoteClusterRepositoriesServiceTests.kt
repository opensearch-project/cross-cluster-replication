...
class RemoteClusterRepositoriesServiceTests : OpenSearchTestCase() {
    ...
    fun `test changes in seed_nodes`() {
        ...
        clusterSetting.applySettings(
            Settings.builder().putList("cluster.remote.con-alias.seeds", "127.0.0.1:9300", "127.0.0.2:9300").build(),
        )
        Mockito
            .verify(
                repositoriesService,
                times(1),
            ).registerInternalRepository(REMOTE_REPOSITORY_PREFIX + "con-alias", REMOTE_REPOSITORY_TYPE)
        threadPool.shutdown()
    }
    ...
    fun `test changes in proxy_id for proxy-setup`() {
        ...
        clusterSetting.applySettings(
            Settings
                .builder()
                .put(
                    "cluster.remote.con-alias.mode",
                    "proxy",
                ).put("cluster.remote.con-alias.proxy_address", "127.0.0.1:100")
                .build(),
        )
        Mockito
            .verify(
