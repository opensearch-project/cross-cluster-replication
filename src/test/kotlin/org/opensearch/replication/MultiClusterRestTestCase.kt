...
abstract class MultiClusterRestTestCase : OpenSearchTestCase() {
    class TestCluster(
        ...
        init {
            ...
            val builder =
                RestClient.builder(*httpHosts.toTypedArray()).setHttpClientConfigCallback { httpAsyncClientBuilder ->
    ...
    companion object {
        ...
        internal fun createTestCluster(configuration: ClusterConfiguration): TestCluster =
            createTestCluster(
                ...
                configuration.preserveIndices,
                configuration.preserveClusterSettings,
                configuration.forceInitSecurityConfiguration,
            )
        ...
        internal fun createTestCluster(
            ...
            return TestCluster(
                ...
                securityEnabled.equals("true", true),
            )
        }
        ...
        protected fun configureClient(
            ...
            if (keystorePath != null) {
                val keystorePass =
                    settings[OpenSearchRestTestCase.TRUSTSTORE_PASSWORD]
                ...
                check(Files.exists(path)) { OpenSearchRestTestCase.TRUSTSTORE_PATH + " is set but points to a non-existing file" }
                try {
            ...
            if (securityEnabled) {
                defaultHeaders[i++] =
                    BasicHeader("Authorization", "Basic " + Base64.getEncoder().encodeToString(creds.toByteArray(StandardCharsets.UTF_8)))
            }
    ...
    private fun registerSnapshotRepository(testCluster: TestCluster) {
        val getResponse: Map<String, Any> =
            OpenSearchRestTestCase.entityAsMap(
                testCluster.lowLevelClient.performRequest(Request("GET", "/_cluster/settings?include_defaults=true&flat_settings=true")),
            )
    ...
    protected fun setupDefaultSecurityRoles(testCluster: TestCluster) {
        ...
        triggerRequest(
            ...
            "PUT",
            "_plugins/_security/api/roles/leader_role",
            leaderRoleConfig,
        )
        ...
        triggerRequest(
            testCluster.lowLevelClient,
            "PUT",
            "_plugins/_security/api/roles/follower_role",
            followerRoleConfig,
        )
        ...
        triggerRequest(
            testCluster.lowLevelClient,
            "PUT",
            "_plugins/_security/api/rolesmapping/leader_role",
            userMapping,
        )
        ...
        triggerRequest(
            testCluster.lowLevelClient,
            "PUT",
            "_plugins/_security/api/rolesmapping/follower_role",
            userMapping,
        )
    ...
    private fun waitForSnapshotWiping(testCluster: TestCluster) {
        ...
        try {
            ...
            assertBusy({
                snapshots.set(wipeSnapshots(testCluster))
    ...
    protected fun wipeClusterSettings(testCluster: TestCluster) {
        val getResponse: Map<String, Any> =
            OpenSearchRestTestCase.entityAsMap(
                testCluster.lowLevelClient.performRequest(Request("GET", "/_cluster/settings")),
            )
        var mustClear = false
    ...
    private fun stopAllReplicationJobs(testCluster: TestCluster) {
        ...
        var parser =
            XContentType.JSON.xContent().createParser(
                ...
                indicesResponseEntity,
            )
        parser.list().forEach { item ->
            val str = item.toString()
            val map =
                str.subSequence(1, str.length - 1).split(",").associate {
    ...
    protected fun wipeIndicesFromCluster(testCluster: TestCluster) {
        ...
        try {
            ...
            response.entity.content.use { `is` ->
                assertTrue(XContentHelper.convertToMap(XContentType.JSON.xContent(), `is`, true)["acknowledged"] as Boolean)
            }
        } catch (e: ResponseException) {
    ...
    protected fun wipeSnapshots(testCluster: TestCluster): Map<String, List<Map<String, Any>>> {
    ...
    protected fun deleteRepository(
    ...
    fun getNamedCluster(clusterName: String): TestCluster =
        testClusters[clusterName] ?: error(
    ...
    fun getClientForCluster(clusterName: String): RestHighLevelClient = getNamedCluster(clusterName).restClient

    fun getAsMap(
        client: RestClient,
        endpoint: String,
    ): Map<String, Any> = OpenSearchRestTestCase.entityAsMap(client.performRequest(Request("GET", endpoint)))

    fun getAsList(
        client: RestClient,
        endpoint: String,
    ): List<Any> = OpenSearchRestTestCase.entityAsList(client.performRequest(Request("GET", endpoint)))

    protected fun deleteConnection(
        fromClusterName: String,
        connectionName: String = "source",
    ) {
    ...
    protected fun createConnectionBetweenClusters(
        fromClusterName: String,
    ...
    protected fun getPrimaryNodeForShard(
        clusterName: String,
        indexname: String,
        shardNumber: String,
    ): String {
        ...
        parser.list().forEach { item ->
            ...
            val map =
                entryValue.subSequence(1, entryValue.length - 1).split(",").associate {
                    val (key, value) = it.trim().split("=")
                    key to value
                }
    ...
    fun getReplicationTaskList(
        clusterName: String,
        action: String = "*replication*",
    ): List<TaskInfo> {
    ...
    protected fun insertDocToIndex(
        clusterName: String,
        docCount: String,
        docValue: String,
        indexName: String,
    ...
    protected fun setMetadataSyncDelay() {
        ...
        updateSettingsRequest.transientSettings(
            Collections.singletonMap<String, String?>(ReplicationPlugin.REPLICATION_METADATA_SYNC_INTERVAL.key, "5s"),
        )
...
//    TODO Find a way to skip tests when tests are run for remote clusters
    ...
    protected fun isMultiNodeClusterConfiguration(
    ...
    protected fun docCount(
        cluster: RestHighLevelClient,
        indexName: String,
    ): Int {
        val persistentConnectionRequest = Request("GET", "/$indexName/_search?pretty&q=*")

        val persistentConnectionResponse = cluster.lowLevelClient.performRequest(persistentConnectionRequest)
        val statusResponse: Map<String, Map<String, Map<String, Any>>> =
            OpenSearchRestTestCase.entityAsMap(
    ...
    protected fun deleteIndex(
        testCluster: RestHighLevelClient,
        indexName: String,
    ) {
