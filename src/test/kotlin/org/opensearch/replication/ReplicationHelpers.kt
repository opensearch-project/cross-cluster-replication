...
data class UseRoles(
    val leaderClusterRole: String = "leader_role",
    val followerClusterRole: String = "follower_role",
)
...
fun RestHighLevelClient.replicationStatus(
    index: String,
    verbose: Boolean = true,
    requestOptions: RequestOptions = RequestOptions.DEFAULT,
): Map<String, Any> {
    var lowLevelReplStatusRequest =
        if (!verbose) {
            Request(
                "GET",
                REST_REPLICATION_STATUS.replace("{index}", index, true),
            )
        } else {
            Request("GET", REST_REPLICATION_STATUS_VERBOSE.replace("{index}", index, true))
        }
...
fun RestHighLevelClient.getReplicationTasks(): Map<String, Map<String, String>>? {
    ...
    val tasks =
        OpenSearchRestTestCase.entityAsMap(lowLevelStatusResponse) as
...
fun `validate paused status response`(
...
fun RestHighLevelClient.stopReplication(
    index: String,
...
fun RestHighLevelClient.pauseReplication(
    index: String,
    reason: String? = null,
    shouldWait: Boolean = true,
    requestOptions: RequestOptions = RequestOptions.DEFAULT,
...
fun RestHighLevelClient.resumeReplication(
    index: String,
    requestOptions: RequestOptions = RequestOptions.DEFAULT,
) {
...
fun RestHighLevelClient.updateReplication(
    index: String,
    settings: Settings,
    requestOptions: RequestOptions = RequestOptions.DEFAULT,
) {
...
fun RestHighLevelClient.waitForReplicationStart(
    index: String,
    waitFor: TimeValue = TimeValue.timeValueSeconds(10),
) {
    assertBusy({
        ...
        val request =
            ListTasksRequest().setDetailed(true).setActions(
...
fun RestHighLevelClient.waitForShardTaskStart(
    index: String,
    waitFor: TimeValue = TimeValue.timeValueSeconds(10),
) {
    assertBusy({
...
fun RestHighLevelClient.waitForReplicationStop(
    index: String,
    waitFor: TimeValue = TimeValue.timeValueSeconds(30),
) {
    assertBusy({
...
fun RestHighLevelClient.deleteAutoFollowPattern(
    connection: String,
    patternName: String,
) {
