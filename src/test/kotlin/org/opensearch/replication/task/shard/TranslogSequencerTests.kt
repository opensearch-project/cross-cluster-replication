...
class TranslogSequencerTests : OpenSearchTestCase() {
    ...
    val replicationMetadata =
        ReplicationMetadata(
            ...
            "test user",
            ReplicationContext(followerShardId.indexName, null),
            ReplicationContext(leaderIndex, null),
            Settings.EMPTY,
        )
    ...
    @Test
    fun `test sequencer out of order`() =
        runTest {
    ...
    fun randomChangesResponse(startSeqNo: Long): Pair<GetChangesResponse, Long> {
        ...
        val changes =
            randomList(1, randomIntBetween(1, 512)) {
                seqNo = seqNo.inc()
                Translog.Index(
