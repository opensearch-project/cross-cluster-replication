...
class IndexReplicationTaskTests : OpenSearchTestCase() {
    ...
    fun testExecute() = runBlocking {
        ...
        val restoreEntry = RestoreInProgress.Entry(
            "restoreUUID", snapshot, RestoreInProgress.State.INIT, Collections.unmodifiableList(ArrayList<String>(indices)),
            null,
        )
        ...
        builder.putCustom(
            ...
            RestoreInProgress.Builder(
                state.custom(RestoreInProgress.TYPE, RestoreInProgress.EMPTY),
            ).add(restoreEntry).build(),
        )
    ...
    fun testStartMissingShardTasks() = runBlocking {
        ...
        tasks.addTask<PersistentTaskParams>(
            "replication:0", ShardReplicationExecutor.TASK_NAME, ShardReplicationParams("remoteCluster", sId, sId),
    ...
    fun testIsTrackingTaskForIndex() = runBlocking {
        ...
        tasks.addTask<PersistentTaskParams>(
            "replication:0", IndexReplicationExecutor.TASK_NAME, IndexReplicationParams("remoteCluster", leaderIndex, followerIndex),
    ...
    private fun createIndexReplicationTask(): IndexReplicationTask {
        ...
        val indexReplicationTask = IndexReplicationTask(
            1, "type", "action", "description", EMPTY_TASK_ID,
