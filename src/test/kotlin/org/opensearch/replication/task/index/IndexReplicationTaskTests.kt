...
class IndexReplicationTaskTests : OpenSearchTestCase() {
        ...
        runBlocking {
            ...
            val restoreEntry =
                RestoreInProgress.Entry(
                    ...
                    Collections.unmodifiableList(ArrayList<String>(indices)),
                    null,
                )
            ...
            metaBuilder.put(
                IndexMetadata
                    .builder(REPLICATION_CONFIG_SYSTEM_INDEX)
            ...
            builder.putCustom(
                ...
                RestoreInProgress.Builder(state.custom(RestoreInProgress.TYPE, RestoreInProgress.EMPTY)).add(restoreEntry).build(),
            )
            ...
            builder.putCustom(
                RestoreInProgress.TYPE,
                RestoreInProgress.Builder(state.custom(RestoreInProgress.TYPE, RestoreInProgress.EMPTY)).build(),
            )
    ...
    fun testStartMissingShardTasks() =
        runBlocking {
            val replicationTask: IndexReplicationTask = spy(createIndexReplicationTask())
            var taskManager = Mockito.mock(TaskManager::class.java)
            replicationTask.setPersistent(taskManager)
    ...
    fun testIsTrackingTaskForIndex() =
        runBlocking {
            val replicationTask: IndexReplicationTask = spy(createIndexReplicationTask())
            var taskManager = Mockito.mock(TaskManager::class.java)
            replicationTask.setPersistent(taskManager)
            ...
            tasks.addTask<PersistentTaskParams>(
                ...
                IndexReplicationParams("remoteCluster", leaderIndex, followerIndex),
    ...
    private fun createIndexReplicationTask(): IndexReplicationTask {
        ...
        val indexReplicationTask =
            IndexReplicationTask(
                ...
                "description",
                EMPTY_TASK_ID,
                ReplicationPlugin.REPLICATION_EXECUTOR_NAME_FOLLOWER,
                clusterService,
                threadPool,
                spyClient,
    ...
    fun testConditionallyOpenIndex() =
        runBlocking {
            val replicationTask: IndexReplicationTask = spy(createIndexReplicationTask())
